...
export default class Host implements IHost {
  ...
  private appRouteMap = new Map<IApp, IAppRouteInfo>();
  /**
   * 已经加载的应用
   */
  ...
  get activePath() {
    return isNullOrEmpty(location.hash) ? "/" : location.hash.substring(1);
  }
  ...
  constructor(load: ILoad) {
    ...
    window.addEventListener("hashchange", async (e) => {
      ...
      if (matched) {
        let [path, paramsString] = this.activePath.split("?");
        let params = {};
  ...
  registerApps(...apps: IAppRouteInfo[]) {
    for (let info of apps) {
      if (!isNullOrEmpty(info.src)) {
  ...
  async getAppByRoute(route: string): Promise<IApp> {
    if (isNullOrEmpty(route)) return Promise.resolve(null);
    ...
    const matched = this.registerdApps
      .filter((app) => {
        ...
        } else if (appRoute instanceof RegExp) {
    ...
    const sortedApps = (apps ?? [])
      .filter(
        (app) =>
          isNullOrEmpty(app.manifest.path) ||
      ...
      .sort(
        ...
        (x, y) => {
          //0:基于匹配的位置
          let xvalue = isNullOrEmpty(x.manifest.path)
            ? Number.MAX_VALUE
            : route.indexOf(x.manifest.path);
          let yvalue = isNullOrEmpty(y.manifest.path)
            ? Number.MAX_VALUE
          ...
          //1:基于路径的深度
          xvalue = isNullOrEmpty(x.manifest.path)
            ? 0
            : x.manifest.path.split("/").length;
          yvalue = isNullOrEmpty(y.manifest.path)
            ? 0
    ...
    if (sortedApps.length > 0) {
      ...
      this.appRouteMap.set(app, matched);
      return app;
    }
    return null;
  }
  ...
  getAppRouteInfo(app: IApp) {
    return this.appRouteMap.get(app);
  }
}
